...
> ⚠ **Feature under development.** A future `get_country_shapes(country)`
> helper will pull World Bank Official Boundaries (with consistent
...
For a quick, non-interactive check, [`pti_plot_boundaries()`](../reference/pti_plot_boundaries.html) draws every admin level as a small-multiple `ggplot` — the same boundary panel the Step 5 data-quality report uses:
...
- Otherwise, jump to [Step 3 — User data](build-pti-3-metadata.html) to prepare your indicator workbook.
