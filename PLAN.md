...
Phase 2.5 §12 bug-fix sprint in progress — **5 of 14 bugs fixed** (#1
`complete_pti_labels` PR #56; #5 `get_adm_levels` PR #57; #6
`get_scores_data` PR #59; #7 `expand_adm_levels` PR TBD, bundled
with the boundary-regex follow-up which is the new 14th §12 row).
Phase 4 (#12, vignettes & pkgdown) and Phase 5 (#13, hex ingestion)
yet to start. R-CMD-check workflow gates merge with `error-on:
'"error"'`. Suite holds at **0 fail / 1 skip / 680 PASS**
(summary-reporter `PASS` count; PR #7 net -2 from the pinned-test
flip + new boundary test).
...
Phase 3  Roxygen2 docs for all permanent fns       (#11)       │  ✓ done
   ...
   └─ Batch 6b Closes Phase 3                            ✓ #53 │
            ▼                                                  │
Phase 2.5  §12 bug-fix sprint (concurrent)         (—)         │  in progress (5/14)
   ...
   ├─ #7 expand_adm_levels >1-slot silent NULL +         ✓ TBD │
   │     boundary-regex follow-up (latent (d))                  │
   └─ #10 #2 #4 #8 #9 #11 #13 #3 #12                     ◌ next │
            ▼                                                  │
...
**Status: 5 of 14 fixed.** Done: #1 `complete_pti_labels` (PR #56),
#5 `get_adm_levels` (PR #57), #6 `get_scores_data` (PR #59),
#7 `expand_adm_levels` (PR TBD), and the boundary-regex follow-up
on the same function (PR TBD; new row added below row #7 in the
same PR — table grew from 13 rows to 14 in lockstep with the
fix). Triage queue (silent data corruption → user-facing errors
→ spec asymmetry): #10 next, then #2 #4 #8 #9 #11 #13, last #3
#12.
