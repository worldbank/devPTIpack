...
| Docs    | `PLAN.md` ticked `mod_DT_inputs_server` under §4.1 1g (2/7 modules covered), bumped Phase-1 ASCII tree to "in progress (2/7)", logged this PR, and added a §11 footnote explaining the suite-total recount: prior `sum(res$nb)` totals (725, 792) over-counted internal `testthat::test_that()` calls inside `validate_geometries()` / `validate_single_geom()`; the summary-reporter `PASS` figure (now 612) is the source of truth going forward |
