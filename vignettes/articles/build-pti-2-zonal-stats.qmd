...
write_xlsx(my_zonal, "sample-data/zonal-stats-adm2.xlsx")
```

Save the file into `sample-data/`, **not** `app-data/` — it is a raw
*input*, not a pipeline output. If you extract indicators at more than
one admin level, write one file per level (e.g. `zonal-stats-adm1.xlsx`,
`zonal-stats-adm2.xlsx`); Step 3 patches each into its matching admin
sheet separately.
...
Step 3 merges this file into the metadata workbook with
[`pti_patch_admin_sheet()`](../reference/pti_patch_admin_sheet.html) —
you never copy values into the Excel by hand. The function left-joins
your zonal table onto the matching `admin<N>_<HumanName>` sheet by
`admin<N>Pcod`, leaving `NA` for any polygon your table doesn't cover.

A typical workflow is:

1. Run this step once per raster indicator, saving each result into `sample-data/`.
2. Declare the new indicator columns in the metadata skeleton's `metadata` sheet — the column names must match the `var_code` values exactly.
3. In Step 3, call `pti_patch_admin_sheet()` once per file to merge the values in.
4. Validate the merged workbook with `validate_metadata()`.
...
When your zonal-stats outputs are ready, continue with [Step 3 — User data](build-pti-3-metadata.html).
