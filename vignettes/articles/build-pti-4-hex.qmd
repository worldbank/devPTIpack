...
`00-master.R` runs this step by default. It is **order-flexible** — it
can run before or after Step 3 — and its output is an independent
intermediate file merged in Step 5. To skip it (e.g. when working
offline), comment out the `04-hex-data.qmd` line in `00-master.R`.
...
To query the **live** data source for the actual years present (not just
the registry hint):
...
Only hexagons matching `hex_ids` are fetched — the request is scoped to
your grid. Temporal variables are pivoted wide:
...
Hex-derived indicators are aggregated onto **every** admin-level sheet
in the output Excel (admin1, admin2, etc.). The `spatial_level` recorded
for each indicator in the `metadata` sheet depends on
`INCLUDE_HEX_IN_APP`:

- `INCLUDE_HEX_IN_APP = TRUE` — the `admin9_Hexagon` sheet ships, and
  `spatial_level` is `admin9_Hexagon`.
- `INCLUDE_HEX_IN_APP = FALSE` (default) — no hex sheet ships, so
  `spatial_level` is set to the finest admin level actually written
  (e.g. `admin2_District`). This keeps the `metadata` sheet consistent
  with the data sheets present in the workbook.
