...
Each layer is a single `sf` tibble. The list slot name follows the package convention `admin<N>_<HumanName>`, e.g. `admin1_Province`. Required columns at minimum:
...
geoBoundaries gives every polygon a `shapeID` and `shapeName`. Rename those into the package convention and compute `area` (`m²` at first; convert if needed):
