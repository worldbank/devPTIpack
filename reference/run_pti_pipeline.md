...
standardise -\> expand -\> merge -\> aggregate -\> label -\> structure)
as a single callable, mirroring the reactive chain in
\`mod_calc_pti2_server()\` but free of any Shiny dependency. Designed
for testing and headless reproduction of the calculation a deployed app
would perform for a given weight scheme.
...
- weights_clean:
  ...
  Named list of weight tibbles, one per weight scheme. Each element must
  be a tibble with character column \`var_code\` and numeric column
  \`weight\`. Compatible with the output of \[get_rand_weights()\] and
  \[get_all_weights_combs()\].

- inp_dta:

  The metadata list as returned by \[fct_template_reader()\] – contains
  admin-level sheets (\`admin\<N\>\_\*\`), \`metadata\`, and
  \`general\`.
...
- indicators_list:
  ...
  Indicator definitions as a tibble. Defaults to
  \`get_indicators_list(inp_dta)\`.

- na_rm:

  Logical. Forwarded to \`agg_pti_scores()\` as \`na_rm_pti2\` – if
  \`TRUE\`, missing indicator values are ignored when row-summing
  standardised contributions to the PTI score.

## Value

A named list with one element per admin level; each element is a list
with \`pti_data\` (an \`sf\` tibble), \`pti_codes\` (named character),
